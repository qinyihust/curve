...
DEFINE_int32(raftMaxSegmentSize, 16 * 1024 * 1024 /*16M*/, 
             "Max size of one segment file");

namespace curve {
namespace chunkserver {

static bvar::LatencyRecorder g_write_entries_latency(
                                        "curve_raft_write_entries");
static bvar::LatencyRecorder g_sync_entries_latency(
                                        "curve_raft_sync_entries");
    ...
    while (dir_reader.Next()) {
        ...
        if (match == 2) {
            ...
            CurveSegment* segment = new CurveSegment(_path, first_index,
                                                     last_index, _checksum_type, false);
        ...
        if (match == 1) {
            BRAFT_VLOG << "restore open segment, path: " << _path
                << " first_index: " << first_index;
            if (!_open_segment) {
                _open_segment = new CurveSegment(_path, first_index, _checksum_type, false);
                continue;
            } else {
        ...
        if (match == 2) {
            LOG(INFO) << "restore closed segment, path: " << _path
                      ...
                      << " last_index: " << last_index;
            CurveSegment* segment = new CurveSegment(_path, first_index,
                                                     last_index, _checksum_type, true);
            _segments[first_index] = segment;
            continue;
        }
        ...
        if (match == 1) {
            BRAFT_VLOG << "restore open segment, path: " << _path
                << " first_index: " << first_index;
            if (!_open_segment) {
                _open_segment = new CurveSegment(_path, first_index, _checksum_type, true);
                continue;
            } else {
                LOG(WARNING) << "open segment conflict, path: " << _path
                    << " first_index: " << first_index;
...
int CurveSegmentLogStorage::append_entries(
    ...
    scoped_refptr<CurveSegment> last_segment = NULL;
    for (size_t i = 0; i < entries.size(); i++) {
        ...
        int ret = segment->append(entry);
        timer.stop();
        g_write_entries_latency << timer.u_elapsed();
        if (0 != ret) {
            return i;
        }
        _last_log_index.fetch_add(1, butil::memory_order_release);
    ...
    timer.stop();
    g_sync_entries_latency << timer.u_elapsed();
    return entries.size();
}
...
void CurveSegmentLogStorage::sync() {
    std::vector<scoped_refptr<CurveSegment> > segments;
    {
        BAIDU_SCOPED_LOCK(_mutex);
...
scoped_refptr<CurveSegment> CurveSegmentLogStorage::open_segment() {
    scoped_refptr<CurveSegment> prev_open_segment;
    {
        BAIDU_SCOPED_LOCK(_mutex);
        if (!_open_segment) {
            _open_segment = new CurveSegment(_path, last_log_index() + 1, _checksum_type, true);
            if (_open_segment->create() != 0) {
                _open_segment = NULL;
                return NULL;
            }
        }
        if (_open_segment->bytes() > FLAGS_raftMaxSegmentSize) {
